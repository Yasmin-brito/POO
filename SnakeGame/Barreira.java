...
public class Barreira implements Elementos {
    ...
    public void adicionaBarreiraAleatoria(int painelWidth, int painelHeight, int passo, ArrayList<Point> snakeSegments, int macaX, int macaY){
       ...
       while(true){
            ...
            Point pontoInicial = new Point(x, y);
            boolean colisao = false;

            for(Point p : snakeSegments){
                if(pontoInicial.equals(p)){
                    colisao = true;
                    break;
                }
            }
       ...
       for(int i = 0; i < 3; i++){
            ...
            if (novoX < painelWidth - passo && novoY < painelHeight - passo && novoY > 50) {
                blocos.add(new Point(novoX, novoY));
            }
        }
    }
